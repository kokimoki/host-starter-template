...
- Updated `@kokimoki/app` to `3.1.7`.

## [1.1.3] - 2026-06-09

### Added

- Added `concept-review` skill for reviewing Kokimoki app/game scalability, state management, synchronization, and performance patterns.

### Changed

- Updated `AGENTS.md` with simplified repository guidance and current Kokimoki SDK documentation entrypoints.
- Updated `@kokimoki/kit` to `1.8.3` and other dependencies with minor version bumps.
...
- Added `eslint-plugin-react-refresh` plugin support
- Added `eslint-plugin-valtio` plugin support

### Changed

- Import `z` from `@kokimoki/app` instead of `@kokimoki/kit` (zod is now re-exported from app)
- Update `@kokimoki/app` to `3.1.6`
- Update `@kokimoki/kit` to `1.8.2`
- Update `@kokimoki/react-components` to `1.0.2`

## [1.1.0] - 2026-02-14

### Added

- Added more code documentations
- Updated `host-starter-template.instructions.md`

### Changed

- Changed global store to shared domain-specific stores
- Split state actions by domain
